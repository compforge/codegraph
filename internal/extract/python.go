...
// Its bounded expression vocabulary is interpreted by consumers; the graph
// model itself never depends on statement-level facts.
...
func pythonProgram(f *Facts, tree *gts.Tree) []PythonStatement {
	lang := tree.Language()
	imports := map[int][]Import{}
	for _, imp := range f.Imports {
		imports[imp.Start] = append(imports[imp.Start], imp)
	}
	remaining := 10000
	exhausted := false
	take := func() bool {
	...
	var expr func(*gts.Node, int) PythonExpression
	expr = func(n *gts.Node, depth int) PythonExpression {
		if n == nil {
	...
	var statements func(*gts.Node, int) []PythonStatement
	statements = func(n *gts.Node, depth int) []PythonStatement {
		if n == nil {
			return nil
		}
		if depth > 64 || !take() {
		...
		case "import_statement", "import_from_statement", "future_import_statement":
			s.Imports = imports[int(n.StartByte())]
